...
WorldLoaded = function()
	...
	ObjectivePreserveSEALs = Greece.AddSecondaryObjective("Keep all SEALs alive.")
	...
	if #CoopPlayers >= 2 then
		Seal1.Owner = CoopPlayers[2]
		Seal2.Owner = CoopPlayers[2]
	end
	
	if #CoopPlayers >= 3 then
		Seal2.Owner = CoopPlayers[3]
	end
	
	if #CoopPlayers >= 4 then
		Spy2 = Actor.Create(Spy.Type, true, { Owner = CoopPlayers[4], Location = Spy.Location })
		RespawnTrigger(Spy2)
		Spy2.Scatter()
	end
	
	if #CoopPlayers >= 5 then
		Seal3 = Actor.Create(Seal1.Type, true, { Owner = CoopPlayers[5], Location = Seal1.Location })
		RespawnTrigger(Seal3)
		Seal3.Scatter()
		table.insert(seals,Seal3)
		Sealcounter = 3
	end
	
	if #CoopPlayers >= 6 then
		Seal4 = Actor.Create(Seal2.Type, true, { Owner = CoopPlayers[6], Location = Seal2.Location })
		RespawnTrigger(Seal4)
		Seal4.Scatter()
		table.insert(seals,Seal4)
		Sealcounter = 4
	end
	...
	Utils.Do(seals, function(a)
		Trigger.OnKilled(a, function(self, killer)
			...
			--[[if Seal1.IsDead and Seal2.IsDead then
				if Seal3 ~= nil and Seal3.IsDead then
				BothSealsDead = true
			end]]
			if Sealcounter <= 0 then
				BothSealsDead = true
			end
...
DoShoreSAMFlare = function()
	Trigger.AfterDelay(DateTime.Seconds(4), function()
		ShoreSAMFlare = Actor.Create("flare", true, { Owner = DummyGuy, Location = ShoreSAMFlareLocation })
		Notification("Signal flare detected. A shore SAM Site has been located.")
		MediaCA.PlaySound("r_samlocated.aud", 2)
		Beacon.New(DummyGuy, ShoreSAMBeaconPosition)
	...
	if CamlockEnabled then
		Utils.Do(CoopPlayers,function(PID)
			...
			if PID.IsLocalPlayer and #ValidCamTargets == 1 then
				...
				--Camera.Position = PID.GetGroundAttackers()[1].CenterPosition
			end
		end)
	end
...
DropChronoPrison = function()
	ChronoPrisonFlare = Actor.Create("flare", true, { Owner = DummyGuy, Location = CarryallDropPoint.Location })
	Media.PlaySpeechNotification(All, "SignalFlare")

	Trigger.AfterDelay(DateTime.Seconds(1), function()
		Notification("Signal flare detected. Reinforcements inbound.")
		Beacon.New(DummyGuy, CarryallDropPoint.CenterPosition)
	end)
	...
	Trigger.AfterDelay(DateTime.Seconds(10), function()
		...
		Trigger.OnEnteredProximityTrigger(CarryallDropPoint.CenterPosition, WDist.New(2048), function(a, id)
			if IsOwnedByCoopPlayer(a) and a.Type == "chpr" then
				Trigger.RemoveProximityTrigger(id)
