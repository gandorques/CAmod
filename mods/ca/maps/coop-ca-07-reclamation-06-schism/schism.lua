...
WorldLoaded = function()
	...
	{
		Mainplayer = USSR,
		MainEnemies = {Nod, ScrinRebels, ScrinRebelsOuter, MaleficScrin},
	...
	Utils.Do(MCVPlayers,function(PID)
		if PID ~= USSR then
			local ExtraMCV = Actor.Create("mcv", true, { Owner = PID, Location = PlayerStart.Location })			
			ExtraMCV.Scatter()
		end
	end)
	...
	if #CoopPlayers >= 2 then
		TeslaPlants[1].Owner = CoopPlayers[2]
		Actor1724.Owner = CoopPlayers[2]
		Actor820.Owner = CoopPlayers[2]
	end
	if #CoopPlayers >= 3 then
		TeslaPlants[2].Owner = CoopPlayers[3]
		Actor1803.Owner = CoopPlayers[3]
		Actor821.Owner = CoopPlayers[3]
	end
	if #CoopPlayers >= 4 then
		TeslaPlants[3].Owner = CoopPlayers[4]
		Actor1610.Owner = CoopPlayers[4]
		Actor822.Owner = CoopPlayers[4]
		Actor823.Owner = CoopPlayers[4]
	end
	if #CoopPlayers >= 5 then
		TeslaPlants[4].Owner = CoopPlayers[5]
		Actor1846.Owner = CoopPlayers[5]
		Actor823.Owner = CoopPlayers[5]
	end
	if #CoopPlayers >= 6 then
		TeslaPlants[5].Owner = CoopPlayers[6]
		Actor1611.Owner = CoopPlayers[6]
	end
	Utils.Do(CoopPlayers,function(PID)
		Actor.Create("hazmatsoviet.upgrade", true, { Owner = PID })
	end)
...
OncePerSecondChecks = function()
	if DateTime.GameTime > 1 and DateTime.GameTime % 25 == 0 then
		...
		if not USSR.IsObjectiveCompleted(ObjectiveSecurePurifier) then
			...
			if IsOwnedByCoopPlayer(Purifier) then
...
PurificationWave = function()
	...
	local exterminators = {}
	Utils.Do(CoopPlayers,function(PID)
		Utils.Do(UID.GetActorsByType("etpd"),function(UID)
			table.insert(exterminators,UID)
		end)
	end)
...
GetPlayerArmyValue = function()
	local value = 0
	Utils.Do(CoopPlayers,function(PID)
		Utils.Do(PID.GetActors(), function(a)
