...
WorldLoaded = function()
	...
	if Multi3 ~= nil then
		local TeamNod = {Nod, Multi3}
		local NodIterator = 1
		local ExtraMCV = Actor.Create(Actor0.Type, true, { Owner = Multi3, Location = Actor0.Location })			
		ExtraMCV.Scatter()
		local NodArmy = Nod.GetGroundAttackers()
		Utils.Do(NodArmy,function(UID)
			UID.Owner = TeamNod[NodIterator]
			NodIterator = NodIterator + 1
			if NodIterator > #TeamNod then
				NodIterator = 1
			end
		end)
	end
...
InitCoopGDI = function()
	TeamGDI = {}
	if Multi1 ~= nil then
		table.insert(TeamGDI, Multi1)
	end
	if Multi4 ~= nil then
		table.insert(TeamGDI, Multi4)
	end
...
InitCoopScrins = function()
	TeamScrinRebels = {ScrinRebels}
	if Multi2 ~= nil then
		table.insert(TeamScrinRebels, Multi2)
	end
	if Multi5 ~= nil then
		table.insert(TeamScrinRebels, Multi5)
	end
	
	Trigger.AfterDelay(1, function()
		if #TeamScrinRebels > 1 then
			local ScrinGuys = Map.ActorsInCircle(Actor160.CenterPosition, WDist.New(18 * 1024), function(UID)
					return UID.Owner == ScrinRebels
				end)
			Utils.Do(ScrinGuys,function(UID)
				UID.Owner = TeamScrinRebels[2]
			end)
			Trigger.AfterDelay(1, function()
				Actor852.Owner = ScrinRebels
				Actor853.Owner = ScrinRebels
			end)
		end
		
		Trigger.AfterDelay(2, function()
			if #TeamScrinRebels > 2 then
				local ScrinGuys = Map.ActorsInCircle(Actor571.CenterPosition, WDist.New(30 * 1024), function(UID)
						return UID.Owner == ScrinRebels
					end)
				Utils.Do(ScrinGuys,function(UID)
					UID.Owner = TeamScrinRebels[3]
				end)
				Trigger.AfterDelay(1, function()
					Actor572.Produce("smcv")
					Actor852.Owner = TeamScrinRebels[3]
				end)
			else if #TeamScrinRebels == 2 then
				local ExtraColony = Actor.Create("sfac", true, { Owner = ScrinRebels, Location = ExtraMCVScrin.Location })	
			end
			end
		end)
	end)
...
OncePerFiveSecondChecks = function()
	if DateTime.GameTime > 1 and DateTime.GameTime % 125 == 0 then
		...
		local ScrinDefeatIterator = 0
		Utils.Do(TeamScrinRebels,function(PID)
			if not PlayerHasBuildings(PID) then
				ScrinDefeatIterator = ScrinDefeatIterator +1
			end
		end)
		if ScrinDefeatIterator >= #TeamScrinRebels and not Victory then
			Nod.MarkFailedObjective(ObjectiveDefendRebels)
		end

		if CoopTeamHasNoRequiredUnits() and not Victory then
...
InitScrin = function()
	...
	Trigger.OnEnteredProximityTrigger(FirstExterminatorDetector.CenterPosition, WDist.New(5 * 1024), function(a, id)
		if a.Owner == Scrin and a.Type == "etpd" then
			...
			Beacon.New(DummyGuy, a.CenterPosition)
...
InitGDI = function()
	if not GDIActive then
		...
		Trigger.AfterDelay(1, function()
			if TeamGDI[2] ~= nil then
				local GDICombatUnits = TeamGDI[1].GetGroundAttackers()
				Utils.Do(GDICombatUnits,function(UID)
					UID.Owner = TeamGDI[2]
				end)
				GDIFactory.Owner = TeamGDI[2]
				Actor1467.Owner = TeamGDI[2]
				GDIFactory.Produce("amcv")
			end
		end)
...
PlayerHasMassAir = function()
	...
	Utils.Do(CoopPlayers,function(PID)
		Utils.Do(PID.GetActorsByTypes({ "scrn", "apch", "venm" }),function(UID)
			table.insert(nodAir,UID)
		end)
	end)
	return #nodAir > 7
end
